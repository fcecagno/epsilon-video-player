...
double alg_house(int n, vector x)   /* Householder変換      */
{
	int i;
	double s, t;
	s = sqrt(alg_innerproduct(n, x, x));  /* 内積の平方根 */
	if (s != 0) {
		if (x[0] < 0) s = -s;
...
void alg_tridiagonalize(int n, matrix a, vector d, vector e) /* 3重対角化 */
{
	int i, j, k;
	double s, t, p, q;
	vector v, w;
	for (k = 0; k < n - 2; k++) {
		...
		t = alg_innerproduct(n-k-1, &v[k+1], &d[k+1]) / 2;
		for (i = n - 1; i > k; i--) {
	...
	for (k = n - 1; k >= 0; k--) {
		...
		if (k < n - 2) {
			for (i = k + 1; i < n; i++) {
				w = a[i];
				t = alg_innerproduct(n-k-1, &v[k+1], &w[k+1]);
				for (j = k + 1; j < n; j++)
					w[j] -= t * v[j];
			}
		}
...
int alg_eigen(int n, matrix a, vector d)
{
	int i, j, k, h, iter;
	double c, s, t, w, x, y;
	vector v, e;
