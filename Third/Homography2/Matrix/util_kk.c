...
double operator,(const matrix& m1, const matrix& m2)
{
  if(m1.rsz() != m2.rsz() || m1.csz() != m2.csz()) {
    cerr << ",(matrix,matrix): dimensional error." << endl;
    exit(1); }
...
{
  if(v1.sz() != 3 || v2.sz() != 3) {
    cerr << "%(vector,vector): dimensional error." << endl;
    exit(1); }
...
matrix operator%(const vector& v, const matrix& m)
{
  if(v.sz() != 3 || m.rsz() != 3 || m.csz() != 3) {
    cerr << "%(vector,matrix): dimensional error." << endl;
    exit(1); }
...
matrix operator%(const matrix& m, const vector& v)
{
  if(m.rsz() != 3 || m.csz() != 3 || v.sz() != 3) {
    cerr << "%(matrix,vector): dimensional error." << endl;
    exit(1); }
...
{
  if(m.rsz() != m.csz()) {
    cerr << "S(matrix): not square matrix." << endl;
    exit(1); }
...
{
  if(m.rsz() != m.csz()) {
    cerr << "A(matrix): not square matrix." << endl;
    exit(1); }
...
{
  if(m.rsz() != 3 || m.csz() != 3) {
    cerr << "t3(matrix): dimensional error." << endl;
    exit(1); }

  if(m[0][0] != 0.0 || m[1][1] != 0.0 || m[2][2] != 0.0 ||
     m[0][1] != -m[1][0] || m[0][2] != -m[2][0] || m[1][2] != -m[2][1]) {
    cerr << "t3(matrix): not anti-symmetric matrix." << endl;
    exit(1); }
