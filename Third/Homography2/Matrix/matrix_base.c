...
{
  if(rsz < 1 || csz < 1) {
    cerr << "matrix::matrix(int,int): invalid dimension." << endl;
    exit(1); }
  ...
  if((_m = new double[_rsz*_csz]) == 0) {
    cerr << "matrix::matrix(int,int): memory allocation error." << endl;
    exit(1); }
...
{
  if(m._m == 0) {
    cerr << "matrix::matrix(matrix): copy null matrix." << endl;
    exit(1); }
  ...
  if((_m = new double[_rsz*_csz]) == 0) {
    cerr << "matrix::matrix(matrix): memory allocation error." << endl;
    exit(1); }
...
{
  if(_m == 0 || m._m == 0) {
    cerr << "matrix::=(matrix): assign null matrix." << endl;
    exit(1); }
  
  if(this == &m) return *this;

  if(_rsz != m._rsz || _csz != m._csz) {
    cerr << "matrix::=(matrix): dimension conflict." << endl;
    exit(1); }
...
{
  if(csz < 1 || rsz < 1) {
    cerr << "matrix::init(int,int): invalid dimension." << endl;
    exit(1); }
  ...
  if((_m = new double[_rsz*_csz]) == 0) {
    cerr << "matrix::init(int,int): memory allocation error." << endl;
    exit(1); }
...
{
  if(m._csz != 1) {
    cerr << "vector::vector(matrix): convert invalid matrix." << endl;
    exit(1); }
...
{
  if(csz != 1) {
    cerr << "vector::init(int,int): invalid dimension." << endl;
    exit(1); }
...
{
  if(v1._rsz != v2._rsz) {
    cerr << "+(vector,vector): dimension conflict." << endl;
    exit(1); }
...
{
  if(m._csz != v._rsz) {
    cerr << "*(matrix,vector): dimension conflict." << endl;
    exit(1); }
