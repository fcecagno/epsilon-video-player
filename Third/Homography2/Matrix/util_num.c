...
matrix inv(const matrix& m)
{
  ...
  // check dimension.
  if(n < 1 || m.csz() != n) {
    cerr << "inv: invalid dimension." << endl;
    exit(1); }

  // memory allocation for buffers.
  if((mm = new double*[n]) == 0 || (imidx = new double*[n]) == 0 ||
     (mm[0] = new double[n*n]) == 0 ) {
    cerr << "inv: memory allocation error." << endl;
    exit(1); }
...
double det(const matrix& m)
{
  ...
  // check dimension.
  if(n < 1 || m.csz() != n) {
    cerr << "det: invalid dimension." << endl;
    exit(1); }

  // memory allocation for buffers.
  if((mm = new double*[n]) == 0 || (mm[0] = new double[n*n]) == 0 ) {
    cerr << "det: memory allocation error." << endl;
    exit(1); }
...
void eigens(const matrix& m, matrix& u, vector& v)
{
  ...
  if(m.csz() != n || u.rsz() != n || u.csz() != n || v.sz() != n) {
    cerr << "eigens: invalid dimension." << endl;
    exit(1); }
  ...
  for(j=1; j<n; j++)
    for(i=0; i<j; i++)
      if(m[j][i] != m[i][j]) {
        cerr << "eigens: not symmetric matrix." << endl;
        exit(1); }

  // allocate memory for work vector.
  if((w = new double[n]) == 0) {
    cerr << "eigens: memory allocation error." << endl;
    exit(1); }
  ...
  if(alg_eigen(n,uptr,&(v[0])) != 0) {
    cerr << "eigens: computation error." << endl;
    exit(1); }
...
matrix invs(const matrix& m, double cn)
{
  ...
  // check dimensions.
  if(n < 1 || m.csz() != n) {
    cerr << "invs: invalid dimension." << endl;
    exit(1); }

  // check designated condition number
  if(cn < 1.0) {
    cerr << "invs: invalid condition number." << endl;
    exit(1); }
...
matrix ginvs(const matrix& m, int rank, double cn)
{
  ...
  // check dimensions.
  if(n < 1 || m.csz() != n) {
    cerr << "ginvs: invalid dimension." << endl;
    exit(1); }
...
double dets(const matrix& m)
{
  ...
  // check dimension.
  if(n < 1 || m.csz() != n) {
    cerr << "dets: invalid dimension." << endl;
    exit(1); }
