...
{
  if(m.rsz() != 1 || m.csz() != 1) {
    cerr << "scalar(matrix): invalid dimension." << endl;
    exit(1); }
...
void set(matrix& m1, const matrix& m2, int sr, int sc)
{
  if(sr < 0 || sc < 0 || sr+m2.rsz() > m1.rsz() || sc+m2.csz() > m1.csz()) {
    cerr << "set(matrix,matrix,int,int): invalid position." << endl;
    exit(1); }
...
void set(vector& v1, const vector& v2, int sr)
{
  if(sr < 0 || sr+v2.sz() > v1.sz()) {
    cerr << "set(vector,vector,int): invalid position." << endl;
    exit(1); }
...
matrix part(const matrix& m, int sr, int sc, int rsize, int csize)
{
  if(sr < 0 || sc < 0 || rsize < 0 || csize < 0 ||
    ...
    cerr << "part(matrix,int,int,int,int): invalid part." << endl;
    exit(1); }
...
vector part(const vector& v, int sr, int rsize)
{
  if(sr < 0 || rsize < 0 || sr+rsize > v.sz()) {
    cerr << "part(vector,int,int): invalid part." << endl;
    exit(1); }
...
double tr(const matrix& m)
{
  ...
  if(m.rsz() != m.csz()) {
    cerr << "tr(matrix): not square matrix." << endl;
    exit(1); }
...
matrix normal(const matrix& m)
{
  ...
  if((x=norm(m)) == 0.0) {
    cerr << "normal(matrix): zero matrix." << endl;
    exit(1); }
...
{
  if (v1.sz() != v2.sz()) {
    cerr << "inpro(vector,vector): dimensional error." << endl;
    exit(1); }
...
{
  if (v1.sz() != 3 || v2.sz() != 3) {
    cerr << "expro(vector,vector): dimensional error." << endl;
    exit(1); }
...
{
  if (d < 1) {
    cerr << "unit(int): invalid dimension." << endl;
    exit(1); }
